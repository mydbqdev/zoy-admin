	...
	public CommonResponseDTO<SupportTicketDTO> zoySupportTicketList(PaginationRequest paginationRequest,boolean isClose,boolean isFinanceUser)  throws WebServiceException {
		try {
			...
			queryBuilder.append(
					...
					+ "        CASE \r\n"
					+ "            WHEN um.user_id IS NULL THEN zpod.pg_owner_name \r\n"
					+ "            ELSE COALESCE(um.user_first_name, '') || ' ' || COALESCE(um.user_last_name, '')\r\n"
					+ "        END AS full_name "
					+ "    FROM pgusers.user_help_request helpreq\r\n"
					+ "    left JOIN pgusers.user_master um ON um.user_id = helpreq.user_id "
					+ "    left join pgowners.zoy_pg_owner_details zpod on zpod.pg_owner_id =helpreq.partner_id "
