...
public class ZoySuperAdminController implements ZoySuperAdminImpl {
	...
	@Override
	public ResponseEntity<String> zoyTenantCardsDetails() {
		ResponseBody response = new ResponseBody();
		try {
			List<Object[]> result=adminDBImpl.findTenantsCardsDetails();
			if (result == null || result.isEmpty()) {
				response.setStatus(HttpStatus.BAD_REQUEST.value());
				response.setError("No data found for Tenant card details.");
				return new ResponseEntity<>(gson.toJson(response), HttpStatus.BAD_REQUEST);
			}
			Object[] row = result.get(0);
			Long activeTenantsCount = (row[0] != null && row[0] instanceof Number) ? ((Number) row[0]).longValue() : 0L;
			Long upcomingTenantsCount = (row[1] != null && row[1] instanceof Number) ? ((Number) row[1]).longValue() : 0L;
			Long inactiveTenantsCount = (row[2] != null && row[2] instanceof Number) ? ((Number) row[2]).longValue() : 0L;
			Long registerTenantsCount = (row[3] != null && row[3] instanceof Number) ? ((Number) row[3]).longValue() : 0L;
			
			TenantCardsDetails superAdminCardsDetails = new TenantCardsDetails(
					 activeTenantsCount,  upcomingTenantsCount,  inactiveTenantsCount,  registerTenantsCount);
			response.setStatus(HttpStatus.OK.value());
			response.setMessage("Successfully fetched Tenant card details.");
			return new ResponseEntity<>(gson.toJson(superAdminCardsDetails), HttpStatus.OK);
		} catch (DataAccessException e) {
			log.error("Database error API:/zoy_admin/tenant_card_details.zoyTenantCardsDetails ", e);
			response.setStatus(HttpStatus.INTERNAL_SERVER_ERROR.value());
			response.setError("Database error occurred while fetching Tenant card details.");
			return new ResponseEntity<>(gson.toJson(response), HttpStatus.INTERNAL_SERVER_ERROR);
		} catch (Exception e) {
			log.error("Unexpected error API:/zoy_admin/tenant_card_details.zoyTenantCardsDetails", e);
			response.setStatus(HttpStatus.INTERNAL_SERVER_ERROR.value());
			response.setError("An unexpected error occurred.");
			return new ResponseEntity<>(gson.toJson(response), HttpStatus.INTERNAL_SERVER_ERROR);
		}
	...
	@Override
	public ResponseEntity<String> zoyOwnerCardsDetails() {
		ResponseBody response = new ResponseBody();
		try {
			List<Object[]> result=adminDBImpl.findOwnerCardsDetails();
			if (result == null || result.isEmpty()) {
				response.setStatus(HttpStatus.BAD_REQUEST.value());
			...
			Object[] row = result.get(0);
			Long registerOwnerDetails = (row[0] != null && row[0] instanceof Number) ? ((Number) row[0]).longValue() : 0L;
			
			OwnerCardDetails ownerCardsDetails = new OwnerCardDetails(registerOwnerDetails);
			response.setStatus(HttpStatus.OK.value());
			response.setMessage("Successfully fetched owner card details.");
			return new ResponseEntity<>(gson.toJson(ownerCardsDetails), HttpStatus.OK);
		} catch (DataAccessException e) {
			log.error("Database error API:/zoy_admin/owner_card_details.zoyOwnerCardsDetails ", e);
			response.setStatus(HttpStatus.INTERNAL_SERVER_ERROR.value());
			response.setError("Database error occurred while fetching Tenant card details.");
			return new ResponseEntity<>(gson.toJson(response), HttpStatus.INTERNAL_SERVER_ERROR);
		} catch (Exception e) {
