...
public class ZoySuperAdminController implements ZoySuperAdminImpl {
	...
	public ResponseEntity<String> zoyTenantCardsDetails() {
		...
		try {
			...
			Long suspendedTenantsCount = (row[3] != null && row[3] instanceof Number) ? ((Number) row[3]).longValue() : 0L;
			
			TenantCardsDetails superAdminCardsDetails = new TenantCardsDetails(
					 activeTenantsCount,  upcomingTenantsCount,  inactiveTenantsCount,  suspendedTenantsCount);
			response.setStatus(HttpStatus.OK.value());
