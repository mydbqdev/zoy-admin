...
public class AuditHistoryUtilities {
	...
	public void auditForCreateVendorUserDelete(String email,boolean isCreate,ZoyVendorMaster vendor) throws WebServiceException {
		try {
			String userName="";
			String operation="";
			Optional<AdminUserMaster> user=userMasterRepository.findById(email);
			...
			String histotyData=null;
			if(isCreate) {
				operation = ZoyConstant.ZOY_ADMIN_VENDOR_USER_CREATE;
				histotyData=userName+" has created the user for, User Name="+vendor.getVendorFirstName()+" "+vendor.getVendorLastName()+", Designation=Vendor, Contact Number="+vendor.getVendorMobileNo()+", Email="+vendor.getVendorEmail();
			} else {
				String statusCode = vendor.getVendorStatus();
				String status;

				if ("P".equalsIgnoreCase(statusCode)) {
					status = "Pending for Approval";
					operation = ZoyConstant.ZOY_ADMIN_VENDOR_USER_PENDING;
				} else if ("A".equalsIgnoreCase(statusCode)) {
					status = "Approved";
					operation = ZoyConstant.ZOY_ADMIN_VENDOR_USER_APPROVED;
				} else if ("R".equalsIgnoreCase(statusCode)) {
					status = "Rejected";
					operation = ZoyConstant.ZOY_ADMIN_VENDOR_USER_REJECTED;
				} else if ("I".equalsIgnoreCase(statusCode)) {
					status = "Inactive";
					operation = ZoyConstant.ZOY_ADMIN_VENDOR_USER_INACTIVE;
				} else {
					status = "Unknown";
					operation = ZoyConstant.ZOY_ADMIN_VENDOR_USER_UNKNOWN;
				}

				histotyData = userName + " has deleted the user for, User Name=" + vendor.getVendorFirstName() + " " + vendor.getVendorLastName() +
						", Designation=Vendor, Contact Number=" + vendor.getVendorMobileNo() + ", Email=" + vendor.getVendorEmail() + ", status=" + status;
			}
			AuditHistory auditHistory = new AuditHistory();
			auditHistory.setUserEmail(email);
			auditHistory.setOperation(operation);
			...
			log.error("Error in audit entry for auditForCreateVendorUserDelete"+email+":",e);
			new ZoyAdminApplicationException(e, "");
		}
	}
}
