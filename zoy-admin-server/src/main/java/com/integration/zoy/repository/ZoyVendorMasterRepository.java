...
public interface ZoyVendorMasterRepository extends JpaRepository<ZoyVendorMaster, String> {
	...
	@Query(value = "SELECT  "
			...
			+ "			zvm.vendor_desgination ,zvm.vendor_group_name  "
			...
			+ "			WHERE zvm.vendor_lead is true and ( "
			+ "    			COALESCE(:searchText, '') = '' or  ( COALESCE(:searchText, '') != '' and ( "
			...
			+ "    			)) "
			+ "			) "
			+ "			AND (COALESCE(:status, '') = '' or ( COALESCE(:status, '') != '' and zvm.vendor_status = :status)) "
			+ "			GROUP BY  "
			+ "			zvm.vendor_id, "
			+ "			zvm.vendor_company_name, "
			+ "			zvm.vendor_first_name, "
			...
			countQuery = "SELECT COUNT(DISTINCT zvm.vendor_id) "
					...
					+ "			AND (COALESCE(:status, '') = '' or ( COALESCE(:status, '') != '' and zvm.vendor_status = :status)) ",
					nativeQuery = true)
	Page<Object[]> findAllVendors(Pageable pageable, String searchText, String status);
}
