...
public interface PgOwnerMaterRepository extends JpaRepository<PgOwnerMaster, String> {
    ...
    @Query(value="select zpod.pg_owner_id,zpod.pg_owner_name ,pom.first_name ,pom.last_name,zpod.pg_owner_email ,zpod.pg_owner_mobile,  "
    		...
    		+ "zppd.property_house_area AS pgaddress, "
	...
	@Query(value = "select   "
			...
			+ "  WHEN EXISTS ( "
			+ "            SELECT 1  "
			+ "            FROM pgadmin.pg_owner_master pom2 "
			+ "            WHERE (pom2.mobile_no = zprod.mobile OR pom2.register_id = zprod.register_id) "
			+ "              AND pom2.zoy_code IS NOT NULL "
			+ "              AND pom2.register_id <> zprod.register_id    "
			+ "        )  "
			+ "        THEN false "
			+ "        ELSE true "
			+ "    END AS initialzoycode, "
			+ "pom.zoy_code,  "
			+ "zprod.status   "
			+ "from pgowners.zoy_pg_registered_owner_details zprod  "
			+ "left join pgsales.properties p on zprod.property_id = p.property_id  "
			+ "left join pgsales.pg_owners po on po.mobile_number = zprod.mobile  "
			+ "left join pgowners.zoy_pg_owner_details zpod on zpod.pg_owner_mobile =po.mobile_number   "
			+ "left join pgadmin.pg_owner_master pom on pom.register_id = zprod.register_id   "
			+ "where zprod.register_id=:ownerDetails",
	        nativeQuery = true)
